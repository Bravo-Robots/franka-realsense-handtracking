...
class HandCV(Node):
    def __init__(self):
        ...
        self.hand_scale = 0.005

        self.filtered_z = None
        self.z_alpha = 0.2
        self.z_max_jump = 50.0
        self.z_window = 6
        self.z_min_valid_pixels = 10
        self.dm_last = None
    ...
    def process_depth_image(self, annotated_image=None, detection_result=None):
        ...
        u = int(np.clip(self.centroid[0], 0, self.depth_image.shape[1] - 1))
        v = int(np.clip(self.centroid[1], 0, self.depth_image.shape[0] - 1))

        win = self.z_window
        u0 = max(0, u - win)
        u1 = min(self.depth_image.shape[1], u + win + 1)
        v0 = max(0, v - win)
        v1 = min(self.depth_image.shape[0], v + win + 1)
        ...
        if valid.size > 0:
            d_m = float(np.mean(valid)) * 0.1
            if self.dm_last is None:
                self.dm_last = d_m
            if d_m > self.dm_last + 30.0:
                # self.get_logger().info(f"Descarto salto en Z: d_m={round(d_m, 3)}, dm_last={round(self.dm_last, 3)}")
                d_m = self.dm_last
            self.dm_last = d_m
        ...
        (x0, y0) = (int(self.centroid[0]) - 100, int(self.centroid[1]) + 40)
        ...
        line1 = f"(x: {np.round(self.centroid[0] - self.image_width/2)}, y: {np.round(self.centroid[1] - self.image_height/2)}, z: {np.round(self.centroid[2])})"
        line2 = f"Right Gesture: {str(right_gesture)}"

        # alto de la primera línea + un margen
        (_, h1), _ = cv.getTextSize(line1, font, scale, thickness)
        line_spacing = h1 + 8

        cv.putText(annotated_image, line1, (x0, y0), font, scale, color, thickness, cv.LINE_AA)
        cv.putText(annotated_image, line2, (x0, y0 + line_spacing), font, scale, color, thickness, cv.LINE_AA)
